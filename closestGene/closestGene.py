...
#argumentos: g p scores qvalues n_permut
...
# Se construyen las distribuciones de distancia.
peak_gene=[]
...
for i in range(len(Genes)):
    ...
    for a in range(len(center)):
        if chro[a]==Chr[i]:
            if center[a] > down and center[a] < TSSs[i]:
                D.append(abs(center[a]-TSSs[i]))
            if center[a] < up and center[a] > TSSs[i]:
                U.append(abs(center[a]-TSSs[i]))
